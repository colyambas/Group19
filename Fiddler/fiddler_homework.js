...
class Handlers
{
    ...
    */
    ...
    static function OnBeforeResponse(oSession: Session) {
        ...
        // change Подменить параметры запроса в Charles так, чтобы в Postman пришел ответ 
        //где другое name, daily_food > weight из запроса, а daily_sleep < weight из запроса.
        
        if (oSession.uriContains("http://162.55.220.72:5005/object_info_1")){  
            ...
            // этот декодер не сработал, возвращает пустой объект JSON
            //	var j = Fiddler.WebFormats.JSON.JsonDecode(oBody);
	
            /* проверка работы декодера		
            FiddlerApplication.Log.LogString(j);
            //	FiddlerApplication.Log.LogString(j["url"]);
            FiddlerApplication.Log.LogString(oBody);
            FiddlerApplication.Log.LogString(j.ToString["daily_food"]);
            FiddlerApplication.Log.LogFormat(json.JSONObject["daily_food"]);
            */
            // взаимно заменяем значения и меняем имя    
            var temp =  json.JSONObject["daily_food"];
            FiddlerApplication.Log.LogString(temp);
            json.JSONObject["daily_food"] = json.JSONObject["daily_sleep"];
            json.JSONObject["daily_sleep"] = temp;
            json.JSONObject["name"] = "Fedotov";
			
            // Convert back to a byte array
            var modBytes = Fiddler.WebFormats.JSON.JsonEncode(json.JSONObject);

            // Convert json to bytes, storing the bytes in response body
            var mod = System.Text.Encoding.UTF8.GetBytes(modBytes);
            oSession.ResponseBody = mod; 
        }     
		
        // меняет json key в ответе "qa_salary_after_1.5_year" на 
		// "qa_salary_after_1.5_month"
		if (oSession.uriContains("http://162.55.220.72:5005/user_info_2")){  
			oSession["ui-backcolor"] = "blue";
			oSession["ui-bold"] = "Bold text here"
        
			// Convert the response body into a string
			var oBodyUI2 = System.Text.Encoding.UTF8.GetString(oSession.responseBodyBytes);
			// Convert the text into a JSON object
			var sInputUI2 = oSession.GetResponseBodyAsString();
			var Result = sInputUI2.Replace("qa_salary_after_1.5_year", "qa_salary_after_1.5_month");
			var jsonUI2 : JSON.JSONParseResult = JSON.JsonDecode(Result);
			
			//	var js = Fiddler.WebFormats.JSON.JsonDecode(oBodyUI2);
	/* проверка результатов		
			FiddlerApplication.Log.LogString(sInputUI2);
			FiddlerApplication.Log.LogString(Result);
			//	FiddlerApplication.Log.LogString(j["url"]);
			FiddlerApplication.Log.LogString(oBodyUI2);
		//	FiddlerApplication.Log.LogString(js.ToString["qa_salary_after_1.5_year"]);
			FiddlerApplication.Log.LogFormat(jsonUI2.JSONObject["qa_salary_after_1.5_month"]);
	*/	
		
			// Convert back to a byte array
			var modBytesUI2 = Fiddler.WebFormats.JSON.JsonEncode(jsonUI2.JSONObject);

			// Convert json to bytes, storing the bytes in request body
			var modUI2 = System.Text.Encoding.UTF8.GetBytes(modBytesUI2);
			oSession.ResponseBody = modUI2; //RequestBody = mod;
			//	FiddlerApplication.Log.LogString(temp);
		}     
		
	
    }
    //заменяет url в запросе (salary передаем как NULL)
    //чтобы вызвать 500 ошибку
    static function ObjectInfo3Code500 (oSession: Session){
        //   if (oSession.uriContains("http://162.55.220.72:5005/object_info_3")){ 
        //     oSession.utilReplaceInRequest('age=' + '\w', "age=");
        //   }
        oSession.url = "http://162.55.220.72:5005/object_info_3?name=Nick&age=33&salary=";
    }
	
    //заменяем метод запроса с GET на POST, 
    //чтобы вызвать 405 ошибку Method not allowed
    static function ObjectInfo4Code405 (oSession: Session){
    ...
    // меняет json key в ответе "qa_salary_after_1.5_year" на 
    // "qa_salary_after_1.5_month"  
    static function UserInfo2ChangeJsonInResponse (oSession: Session){
        oSession["ui-backcolor"] = "blue";
        oSession["ui-bold"] = "Bold text here"
